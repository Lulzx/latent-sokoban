...
static inline int sk_bfs(const Level *lv, Search *s, const Key *start,
                         int max_nodes, uint8_t *out) {
...
static inline int sk_solve(const Level *lv, Search *s, int max_nodes,
    ...
    for (;;) {
        int n = sk_bfs(lv, s, &start, max_nodes, out);
        if (n != -2) return n;
...
/* Forward transition on a state key (mirrors SokobanEnv.apply). Returns 1 if
 * the move changed the state, 0 for a no-op (wall / blocked push), leaving
 * *out == *in in the no-op case. */
static inline int sk_key_apply(const Level *lv, const Key *in, int action,
                               Key *out) {
    *out = *in;
    int pr = in->player / lv->w, pc = in->player % lv->w;
    int nr = pr + ACT_DR[action], nc = pc + ACT_DC[action];
    if (sk_is_wall(lv, nr, nc)) return 0;
    int ncell = nr * lv->w + nc;
    if (key_has(in, ncell)) {
        int br = nr + ACT_DR[action], bc = nc + ACT_DC[action];
        if (sk_is_wall(lv, br, bc)) return 0;
        int bcell = br * lv->w + bc;
        if (key_has(in, bcell)) return 0;
        key_clear(out, ncell);
        key_set(out, bcell);
    }
    out->player = (uint16_t)ncell;
    return 1;
}

/* Shortest distance from an arbitrary state to the goal (any player position
 * with boxes == goals), or -1 if unreachable / node budget exhausted. This is
 * the distance-to-goal label the world model's value head is trained on.
 * Grows the hash table like sk_solve, so a large search is slow rather than
 * reported unreachable. */
static inline int sk_dist_to_goal(const Level *lv, Search *s, const Key *start,
                                  int max_nodes) {
    for (;;) {
        int n = sk_bfs(lv, s, start, max_nodes, NULL);
        if (n != -2) return n;
        if (s->bits >= SEARCH_MAX_BITS) return -1;
        search_grow(s);
    }
...
/* Reverse transitions (walk-back / pull) on a state key. The reverse of a
 * forward walk is a walk, and the reverse of a forward push is a pull, so the
 * reverse graph's edges are exactly the reversed forward edges. By the
 * graph-reversal theorem the shortest forward distance to the goal equals the
 * shortest reverse distance from the goal, which is what lets one BFS from
 * the goal states label every reachable state at once.
 *
 * Given (player q, boxes B) and direction d, the player's reverse destination
 * is behind = q - d. There are two DISTINCT reverse edges per direction:
 *   - walk: player q -> q-d, boxes unchanged (reverse of a forward walk),
 *   - pull: box at q+d moves to q, player q -> q-d (reverse of a forward push).
 * Both require empty floor at behind; the pull additionally needs a box ahead.
 * A direction can therefore contribute 0, 1, or 2 successors. */
static inline int sk_key_rev_walk(const Level *lv, const Key *in, int action,
                                  Key *out) {
    int pr = in->player / lv->w, pc = in->player % lv->w;
    int br = pr - ACT_DR[action], bc = pc - ACT_DC[action];
    if (sk_is_wall(lv, br, bc)) return 0;
    int bcell = br * lv->w + bc;
    if (key_has(in, bcell)) return 0;
    *out = *in;
    out->player = (uint16_t)bcell;
    return 1;
}

static inline int sk_key_rev_pull(const Level *lv, const Key *in, int action,
                                  Key *out) {
    int pr = in->player / lv->w, pc = in->player % lv->w;
    int br = pr - ACT_DR[action], bc = pc - ACT_DC[action];
    if (sk_is_wall(lv, br, bc)) return 0;
    int bcell = br * lv->w + bc;
    if (key_has(in, bcell)) return 0;
    int acell = (pr + ACT_DR[action]) * lv->w + (pc + ACT_DC[action]);
    if (!key_has(in, acell)) return 0;
    *out = *in;
    key_clear(out, acell);
    key_set(out, pr * lv->w + pc);
    out->player = (uint16_t)bcell;
    return 1;
}

/* Distance map: one reverse BFS from the goal states fills distance-to-goal
 * for every non-dead state of a level. Sized-and-grown like Search, but the
 * per-level cost is a single BFS instead of one per queried state. */
typedef struct {
    Key* keys;
    uint16_t* dist;
    uint32_t* stamp;
...
static inline void distmap_alloc(DistMap* m, int bits) {
    m->bits = bits;
    m->size = 1 << bits;
    m->keys = (Key*)malloc(sizeof(Key) * m->size);
    m->dist = (uint16_t*)malloc(sizeof(uint16_t) * m->size);
    m->stamp = (uint32_t*)calloc(m->size, sizeof(uint32_t));
    m->queue = (int32_t*)malloc(sizeof(int32_t) * m->size);
    m->gen = 0;
}

static inline void distmap_free(DistMap* m) {
    free(m->keys); free(m->dist); free(m->stamp); free(m->queue);
}

static inline void distmap_grow(DistMap* m) {
    int bits = m->bits;
    distmap_free(m);
    distmap_alloc(m, bits + 1);
}

static inline int sk_fill_dist_once(const Level* lv, DistMap* m, int max_nodes) {
    if (++m->gen == 0) {
        memset(m->stamp, 0, sizeof(uint32_t) * m->size);
        m->gen = 1;
    }
    m->qhead = m->qtail = m->filled = 0;

    uint64_t goal_bits[BOX_WORDS] = {0};
    for (int r = 0; r < lv->h; r++)
        for (int c = 0; c < lv->w; c++)
            if (lv->goals[r][c])
                goal_bits[(r * lv->w + c) >> 6] |= 1ULL << ((r * lv->w + c) & 63);

    /* seed every goal state: boxes == goals, player on any free non-goal cell */
    for (int r = 0; r < lv->h; r++)
        for (int c = 0; c < lv->w; c++) {
            if (lv->walls[r][c] || lv->goals[r][c]) continue;
            Key k;
            memset(&k, 0, sizeof(k));
            for (int wd = 0; wd < BOX_WORDS; wd++) k.boxes[wd] = goal_bits[wd];
            k.player = (uint16_t)(r * lv->w + c);
            uint64_t h = key_hash(&k) & (m->size - 1);
            while (m->stamp[h] == m->gen) h = (h + 1) & (m->size - 1);
            m->stamp[h] = m->gen; m->keys[h] = k; m->dist[h] = 0;
            m->queue[m->qtail++] = (int32_t)h;
            m->filled++;
        }

    int nodes = 0;
    while (m->qhead < m->qtail) {
        int32_t cur = m->queue[m->qhead++];
        if (++nodes > max_nodes) return 0;
        uint16_t d = m->dist[cur];
        Key key = m->keys[cur];
        for (int a = 0; a < N_ACTIONS; a++) {
            Key nxs[2];
            int nn = 0;
            if (sk_key_rev_walk(lv, &key, a, &nxs[nn])) nn++;
            if (sk_key_rev_pull(lv, &key, a, &nxs[nn])) nn++;
            for (int i = 0; i < nn; i++) {
                uint64_t slot = key_hash(&nxs[i]) & (m->size - 1);
                int seen = 0;
                while (m->stamp[slot] == m->gen) {
                    if (key_eq(&m->keys[slot], &nxs[i])) { seen = 1; break; }
                    slot = (slot + 1) & (m->size - 1);
                }
                if (seen) continue;
                if (++m->filled > (m->size * 3) / 4) return -2;
                m->stamp[slot] = m->gen; m->keys[slot] = nxs[i];
                m->dist[slot] = (uint16_t)(d + 1);
                if (m->qtail >= m->size) return -2;
                m->queue[m->qtail++] = (int32_t)slot;
            }
        }
    }
    return 0;
...
static inline void sk_fill_dist(const Level* lv, DistMap* m, int max_nodes) {
    for (;;) {
        if (sk_fill_dist_once(lv, m, max_nodes) != -2) return;
        if (m->bits >= 22) return;
        distmap_grow(m);
    }
}

/* Distance-to-goal for a state, or -1 if the state is dead (not reached by
 * the reverse BFS from the goal). */
static inline int sk_dist_lookup(const DistMap* m, const Key* k) {
    uint64_t h = key_hash(k) & (m->size - 1);
    while (m->stamp[h] == m->gen) {
        if (key_eq(&m->keys[h], k)) return m->dist[h];
        h = (h + 1) & (m->size - 1);
    }
    return -1;
}
